...
| total | 944 | | (+29) 973 |
 */
function formatNumber(n) { return Number(n.toFixed(1)).toString(); }
function getPercentage(n, total) { return total == 0 ? 0 : (n * 100 / total); }
function formatPercentage(n, total) { return formatNumber(getPercentage(n, total)) + '%'; }
function getCheck(check, change) {
    return check === 0 ? '' :
        check * change < 0 ? ' :warning:' :
            ' :white_check_mark:';
}
function formatNumberChange(newValue, oldValue, checks = 0, numberDecorator = '', sign = '') {
...
function formatPercentageChange(newValue, newTotal, oldValue, oldTotal, checks = 0, numberDecorator = '') {
    return formatNumberChange(getPercentage(newValue, newTotal), getPercentage(oldValue, oldTotal), checks, numberDecorator, '%');
}
function formatMarkdownReport(target, source) {
